...
Xtrain = np.reshape(Xtrain, (Xtrain.shape[0], Xtrain.shape[2], 5))
Xtest = np.reshape(Xtest, (Xtest.shape[0], Xtest.shape[2], 5))
Ytrain = np.asarray(Ytrain)
Ytest = np.asarray(Ytest)
print(Xtrain.shape)

regressor = Sequential()
regressor.add(Bidirectional(LSTM(units = 128, return_sequences = True, input_shape = (Xtrain.shape[1], 5))))
regressor.add(Dropout(0.2))
regressor.add(Bidirectional(LSTM(units = 64, return_sequences = True))) #this
regressor.add(Dropout(0.2))
